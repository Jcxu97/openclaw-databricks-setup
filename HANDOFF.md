...
> 最后更新：2026-04-29 · 状态：**待开工**（明天执行）
> Skill 文件：`skills/ai-anime-drama-workflow.md`（1394 行，已就绪）

### 目标

用户提供一段小说 → 自动生成 3 分钟 AI 动漫短剧成片。

### 分工

| 角色 | 负责内容 |
| --- | --- |
| **用户** | ① 给小说 ② 注册 API 平台拿 key ③ 确认剧本/角色设计 ④ 最终剪映拼接（~20min） |
| **AI（Claude）** | 剧本 → 角色卡 → 分镜脚本 → Prompt 批量生成 → 写 Python 自动化脚本 → 调 API 生成视频 → 评判质量/重试 → 输出剪辑时间线 |

### 执行步骤（明天开工清单）

#### 第一阶段：准备（用户，10 分钟）

1. **准备小说原文**：粘贴到对话中（建议 1000-3000 字，太长需要分段）
2. **注册 API 平台**（二选一）：
   - **PiAPI**（最便宜，$0.108/秒）：https://piapi.ai → 注册 → 充值 → 拿 API key
   - **SeeGen AI**（有 200 免费 credits）：https://seegen.ai → 注册 → 拿 API key
3. **确认 Python 环境**：本机需要 Python 3.10+（`python --version` 检查）

#### 第二阶段：创意产出（AI，用户确认）

4. **AI 输出剧本初稿**：三幕剧结构 + 分场景描述
5. **用户确认或修改**（5 分钟）
6. **AI 输出角色设定卡**：每个角色的详细外貌/服装/性格
7. **用户确认或修改**（5 分钟）
8. **AI 输出分镜脚本**：12-18 个镜头，每镜头含时长/景别/运镜/动作/台词

#### 第三阶段：自动化生成（AI 全自动）

9. **AI 写 Python 自动化脚本**：
   - 读取 Prompt 表
   - 调用 Seedance/Kling API 逐镜头生成视频
   - 下载结果到本地 `分镜视频/` 目录
   - 自动评判质量，不满意则修改 Prompt 重跑
   - 输出最终文件列表
10. **用户运行脚本**：`python generate.py`（可能需要 1-3 小时，取决于镜头数和重试次数）
11. **AI 检查生成结果**，给出修改建议

#### 第四阶段：成片（用户，20 分钟）

12. **AI 输出剪辑时间线**：每段视频的入出点、转场、调速
13. **用户在剪映中按时间线拼接**
14. **剪映自动识别字幕** → 人工校对
15. **添加 BGM**（可用 Suno AI 生成）
16. **导出成片**
...
| 项目 | 费用 |
| --- | --- |
| API 生成视频（12-18 镜头 × 2-3 次重试） | $30-60 |
| 剪映 | 免费（基础功能够用） |
| BGM（Suno AI） | 免费额度通常够 |
| **总计** | **~$30-60** |
...
| 自动生成（后台跑） | 1-3 小时 |
| 剪映拼接 | 20 分钟 |
| **总计** | **~2-4 小时**（大部分是脚本自动跑的时间）

### 关键文件路径

```
skills/ai-anime-drama-workflow.md    ← 完整 Skill（1394 行）
项目目录/                              ← 生成后自动创建
├── 剧本/故事.txt
├── 剧本/人物设定.txt
├── 剧本/分镜脚本.txt
...
| 平台 | 价格/秒 | 免费额度 | 支持模型 | 推荐理由 |
| --- | --- | --- | --- | --- |
| **PiAPI** | $0.108 (Fast) | 无 | Seedance 2.0 | 最便宜 |
| **SeeGen AI** | $0.128 | 200 credits | Seedance 2.0 | 先试用再决定 |
| **Kling 官方** | 按次计费 | 少量 | Kling 3.0 | 需要对话唇形时用 |

### 注意事项

- 第一次跑建议用 **SeeGen 免费额度**测试效果，满意后再用 PiAPI 批量跑
- API key 存环境变量，不要写进代码或 commit
- Seedance 2.0 高峰期排队 2-10 小时，建议错峰（凌晨/上午）
- 3 分钟短剧 = 120-180 秒视频，按 $0.108/秒算约 $13-20/轮，重试 2-3 轮 = $30-60
